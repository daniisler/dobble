...
def readyBoard(screen, pos, size, players, userId):
    num = len(players)
    place_per_user = size[0] // num
    pygame.draw.rect(screen, (0, 0, 0),pygame.Rect(pos[0], pos[1], size[0], size[1]), 0)
    pygame.draw.rect(screen, (255, 255, 255),pygame.Rect(pos[0], pos[1], size[0], size[1]), 1)
    for player in players:
        if player == userId:
            TextSurf, TextRect = text_objects(player, pygame.font.Font('freesansbold.ttf', 36), (255, 0, 0))
        else:
            TextSurf, TextRect = text_objects(player, pygame.font.Font('freesansbold.ttf', 36))
